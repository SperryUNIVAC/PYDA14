...
/*общая философия запроса такая
 * пишем select и забываем пока про него
 * пишем from и сочиняем конструцию которая подготовит нам данные с джойнами, условиями и тп
 * далее дописываем уже select
 * в общем сам селект не является частью запроса и никак не влияет на обрабатываемые данные (ну если токль мы не хотим 
 * уже ИЗ ЭТИХ обработанных данных что то дополнительно выдрать, подсчитать среднее например*/
 ...
 * distinc
 * order by
 * в итоге несмотря на то что селект у нас первый по написанию, что бы вы там не понабрали все что выше его
 * это не воспримет
 * 
...
/* продвинутая техника
 * уровень 1 - использовать подзапрос в запросе
 * уровень 2 - использовать псевдоним CTE для описания подзапроса и в скрипте таскать с собой
 * уже этот псевдоним
 * уровень 3 - создать View т.е представление - это результат запроса сохраненный
 * в виде таблицы и далее к  нему можно обращаться уже как к нормальной таблице
 * */




/*вывести любую фигню можно довольно просто написав
 * желаемая функция (например count()) далее over(partition by --указать по какому столбцу смотреть - например по ид пользователей)
 * 
 * */
